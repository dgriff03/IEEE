...
\author{
	\IEEEauthorblockN{
		Brett Fischler\IEEEauthorrefmark{1}\IEEEauthorrefmark{3}, 
		Daniel Griffin\IEEEauthorrefmark{1}\IEEEauthorrefmark{4}, 
		Tyler Lubeck\IEEEauthorrefmark{1}\IEEEauthorrefmark{6}, 
		Kenneth Hunter Wapman\IEEEauthorrefmark{1}\IEEEauthorrefmark{4}, 
		Mai Vu PhD\IEEEauthorrefmark{2}\IEEEauthorrefmark{5}
	}
	
	\IEEEauthorblockA{
		Department of Computer Science\IEEEauthorrefmark{1} \& Department of Electrical and Computer Engineering\IEEEauthorrefmark{2}, Tufts University, MA, USA
	}
	
	\IEEEauthorblockA{
		Brett.Fischler@gmail.com\IEEEauthorrefmark{3}, 
		\{Daniel.Griffin, Hunter.Wapman\}@tufts.edu\IEEEauthorrefmark{4},
		Tyler@TylerLubeck.com\IEEEauthorrefmark{6},
		MaiVu@ece.tufts.edu\IEEEauthorrefmark{5}
	}
}
...
\begin{equation}
\label{eq:wknn}
\hat{\rho} = \sum\limits_{i=1}^{K}\frac{\omega_i}{\omega_{total}}\hat{n_i}, \quad w_i = \sigma(n_i, \rho)^{-1}
\end{equation}
where $\omega_i$ is the weight assigned to $n_i$, $\hat{n_i}$ is the position corresponding to $n_i$, $\omega_{total}$ is the sum of the weights of all FPs in N, and $\sigma(n_i, \rho)$ is the signal distance between $n_i$ and $\rho$ using distance metric $\sigma$. The K-Nearest Neighbor (KNN) method is a special case of WKNN which uses equal weights for FPs in $N$. We discuss the specific distance metrics we implement in the next section. We use a K value of 3 because it has been shown to have a higher accuracy than the Nearest Neighbor (NN) method, where K is 1 \cite{Kokkinis}. 
